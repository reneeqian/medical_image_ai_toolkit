...
def test_task_generates_matched_input_target_shapes(evidence_output_dir):
    report = EvidenceReport(
        subject="DAT-009: generate_training_samples yields input and target tensors with matching shapes"
    )
    ...
    report.info(
        f"generate_training_samples: input shape={x.shape}, target shape={y.shape} — match",
        "DAT-009",
    )
...
def test_task_generate_training_samples_interface(evidence_output_dir):
    report = EvidenceReport(
        subject="TRN-008: generate_training_samples yields dict with 'input' and 'target' keys"
    )
...
def test_task_compute_loss(evidence_output_dir):
    ...
    report.info(
        f"compute_loss returned Tensor with value={loss.item():.4f} > 0 for mismatched pred/target",
        "TRN-007",
    )
    report.auto_save("TRN007_task_compute_loss", evidence_output_dir)
    assert not report.has_errors, report.summary()
...
def test_task_slice_level_iteration(evidence_output_dir):
    ...
    report.info(
        f"generate_training_samples yielded {len(outputs)} samples for a 3-slice volume — one sample per slice",
        "DAT-010",
    )
    report.auto_save("DAT010_task_slice_iteration", evidence_output_dir)
    assert not report.has_errors, report.summary()
...
def test_postprocess_prediction_identity(evidence_output_dir):
    ...
    report.info(
        "postprocess_prediction returns input tensor unchanged — default is identity transformation",
        "TRN-008",
    )
    report.auto_save("TRN008_task_postprocess_prediction_identity", evidence_output_dir)
    assert not report.has_errors, report.summary()

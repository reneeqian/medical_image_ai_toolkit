...
def _create_datasource(tmp_path, nan_patient=False):
    ...
    ingestor = SyntheticIngestor(num_patients=10, nan_patient=nan_patient)
...
def test_training_detects_nan_loss(
    ...
    report = EvidenceReport(subject="NaN loss detection during training")

    ds = _create_datasource(tmp_path, nan_patient=True)

    ds.create_partitions(DeterministicSplit())

    model = TinyConvNet()

    config = TrainingConfig(epochs=1, batch_size=2, task=NaNTask())
    ...
    report.info("Trainer correctly detected NaN loss", "TRN-003")

    report.auto_save("TRN003_nan_loss_detection", evidence_output_dir)
...
def test_training_metrics_are_deterministic(tmp_path, evidence_output_dir):
    report = EvidenceReport(
        subject="Two training runs with identical seed produce identical metrics"
    )
    ...
    report.info(
        "Two runs with identical data and seed produced hash-identical metrics.json", "VER-001"
    )
...
def test_training_order_is_deterministic(tmp_path, evidence_output_dir):
    report = EvidenceReport(
        subject="Training epoch history is deterministic given same seed and data order"
    )
...
def test_dataset_partitions_do_not_overlap(
    ...
    report = EvidenceReport(subject="Dataset partition separation enforcement")

    ds = _create_datasource(tmp_path)

    train_ids, val_ids, test_ids = ds.create_partitions(DeterministicSplit())
    ...
    report.info(
        f"No partition overlap: train({len(train_ids)}), val({len(val_ids)}), test({len(test_ids)})",
        "VER-003",
    )
    report.auto_save("VER003_dataset_partition_separation", evidence_output_dir)
...
def test_training_without_partitions_fails(tmp_path, evidence_output_dir):
    ...
    report.auto_save("SYS002_training_without_partitions_fails", evidence_output_dir)
    assert not report.has_errors, report.summary()
...
def test_training_without_task_fails(tmp_path, evidence_output_dir):
    ...
    config = TrainingConfig(epochs=1, task=None)
    ...
    report.info(
        "trainer.train() raised ValueError when task=None — task is required before training",
        "TRN-001",
    )
    report.auto_save("TRN001_training_without_task_fails", evidence_output_dir)
    assert not report.has_errors, report.summary()
...
def test_training_respects_device(tmp_path, evidence_output_dir):
    ...
    config = TrainingConfig(epochs=1, device="cpu", task=DummyTask())
    ...
    report.auto_save("TRN001_training_respects_device", evidence_output_dir)
    assert not report.has_errors, report.summary()
...
def test_training_with_no_patients(tmp_path, evidence_output_dir):
    ...
    class EmptyIngestor:
        ...
        def load_patient_sample(self, pid):
            pass
    ...
    report.info(
        "Training with empty dataset completed successfully with num_train_samples=0", "SYS-002"
    )
    report.auto_save("SYS002_training_with_no_patients", evidence_output_dir)
    assert not report.has_errors, report.summary()
...
def test_early_stop_plateau_halts_training(tmp_path, evidence_output_dir):
    ...
    if results.metrics.get("epochs_trained", 20) >= 20:
        report.error(
            "Training ran to completion despite constant loss (plateau not triggered)", "TRN-005"
        )
    if "plateau" not in (results.metrics.get("early_stop_reason") or ""):
        report.error("early_stop_reason does not name plateau", "TRN-005")

    report.info(
        f"Plateau early stop: epochs_trained={results.metrics.get('epochs_trained')}, reason={results.metrics.get('early_stop_reason')!r}",
        "TRN-005",
    )
...
def test_early_stop_disabled_runs_all_epochs(tmp_path, evidence_output_dir):
    ...
    report.info(
        f"early_stop=False: all 3 epochs ran, early_stop_reason={results.metrics.get('early_stop_reason')!r}",
        "TRN-005",
    )
...
def test_val_loss_appears_in_epoch_history(tmp_path, evidence_output_dir):
    report = EvidenceReport(
        subject="val_loss appears in every epoch history entry when a val partition exists"
    )
    ...
    results = MedicalImageTrainer(
        ...
        TinyConvNet(),
        TrainingConfig(
            epochs=2, task=DummyTask(), early_stop=False, val_evaluator=_DummyEvaluator()
        ),
    ).train()
...
def test_val_evaluator_custom_metrics_appear_in_history(tmp_path, evidence_output_dir):
    report = EvidenceReport(
        subject="val_evaluator aggregate() keys are merged into per-epoch history entries"
    )
    ...
    for entry in results.history:
        if "custom_metric" not in entry:
            report.error(
                f"custom_metric from val_evaluator missing in history entry: {entry}", "TRN-008"
            )
...
def test_training_pipeline_stops_on_dataset_validation_errors(tmp_path, evidence_output_dir):
    ...
    report.info(
        "TrainingPipeline raised RuntimeError('Dataset validation failed') for patient with negative spacing",
        "SYS-005",
    )
    report.auto_save("SYS005_training_pipeline_stops_on_invalid_dataset", evidence_output_dir)
    assert not report.has_errors, report.summary()
...
def test_training_pipeline_writes_model_and_partition_artifacts(tmp_path, evidence_output_dir):
    report = EvidenceReport(
        subject="TrainingPipeline writes model.pt and partitions.json artifacts"
    )

    ds = _create_datasource(tmp_path)
    config = TrainingConfig(
        epochs=1, task=DummyTask(), split_strategy=DeterministicSplit(), early_stop=False
    )
...
def test_training_pipeline_returns_results_with_artifacts(tmp_path, evidence_output_dir):
    ...
    report.info(
        f"TrainingPipeline returned results with artifact keys: {list(results.artifacts)}",
        "SYS-002",
    )
...
def test_checkpoint_save_creates_files(tmp_path, evidence_output_dir):
    report = EvidenceReport(
        subject="checkpoint_every=1 writes per-epoch and latest checkpoint files"
    )
    ...
    report.info(
        "checkpoint_epoch_001.pt, checkpoint_epoch_002.pt, and checkpoint_latest.pt all created with checkpoint_every=1",
        "MOD-002",
    )
...
def test_checkpoint_resume_continues_from_saved_epoch(tmp_path, evidence_output_dir):
    report = EvidenceReport(
        subject="Training resumes from checkpoint and completes remaining epochs"
    )
    ...
    config1 = TrainingConfig(epochs=1, task=DummyTask(), early_stop=False, checkpoint_every=1)
    ...
    report.info(
        f"Checkpoint resume: epochs_trained={epochs_trained} (1 from checkpoint + 1 new)", "MOD-004"
    )
...
def test_sanity_check_reports_partition_sizes(tmp_path, evidence_output_dir):
    ...
    report.info(
        "sanity_check() with partitioned datasource reported partition sizes without error",
        "SYS-003",
    )

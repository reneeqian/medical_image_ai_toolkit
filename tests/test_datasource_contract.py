...
def test_dataset_partition_generation(tmp_path, evidence_output_dir):
    report = EvidenceReport(
        subject="SYS-001: MedicalImageDataSource integrates ingestor, partitioning, and slice access"
    )
...
def test_datasource_len_and_getitem(tmp_path, evidence_output_dir):
    report = EvidenceReport(
        subject="SYS-001: MedicalImageDataSource supports len() and __getitem__ access"
    )
    ...
    if patient.image_volume.shape != (5, 10, 10):
        report.error(
            f"ds[0].image_volume.shape={patient.image_volume.shape}, expected (5,10,10)", "SYS-001"
        )
...
def test_load_slice_shape_and_bounds(tmp_path, evidence_output_dir):
    report = EvidenceReport(
        subject="DAT-004: load_slice returns correct shape and raises IndexError on out-of-bounds index"
    )
...
def test_partition_ids_cover_all_patients(tmp_path, evidence_output_dir):
    report = EvidenceReport(
        subject="DAT-007: train+val+test ids together cover all patients exactly once"
    )
...
def test_get_num_patients_and_get_patient(tmp_path, evidence_output_dir):
    ...
    report.info(
        f"get_num_patients={num}, get_patient shape={patient.image_volume.shape}, get_sample shape={sample.image_volume.shape}",
        "DAT-006",
    )
...
def test_has_partitions_flag(tmp_path, evidence_output_dir):
    ...
    report.info(
        "has_partitions()=False before partitioning; True after create_partitions()", "DAT-007"
    )
...
def test_show_slice_basic(tmp_path, evidence_output_dir):
    ...
    report.info(
        "show_slice valid index succeeded; out-of-bounds raised IndexError; no-selector raised ValueError",
        "DAT-004",
    )
...
def test_show_slice_with_annotations_renders(
    ...
    annotated_ds.show_slice(
        "P0", slice_index=2, cols=1, show_annotations=True, show_bboxes=True, block=True
    )
    annotated_ds.show_slice(
        "P0", slice_range=(1, 3), cols=1, show_annotations=True, show_bboxes=True, block=False
    )
    ...
    report.info(
        "show_slice with annotations, bboxes, slice_index, and slice_range all rendered without error",
        "DAT-004",
    )
...
def test_datasource_get_sample_delegates_to_ingestor(tmp_path, evidence_output_dir):
    report = EvidenceReport(
        subject="DAT-010: datasource.get_sample delegates to ingestor.get_sample when defined"
    )
    ...
    report.info(
        f"get_sample('P2') returned {sample!r} — delegated to ingestor.get_sample", "DAT-010"
    )
...
def test_partitions_are_deterministic(tmp_path, evidence_output_dir):
    ...
    report.info(
        "Two MedicalImageDataSource instances with the same split strategy produced identical partitions",
        "DAT-008",
    )
...
def test_access_before_partition_raises(tmp_path, evidence_output_dir):
    ...
    report.info(
        "get_train_ids() before create_partitions() raised RuntimeError as expected", "DAT-007"
    )
...
def test_invalid_patient_id_raises(tmp_path, evidence_output_dir):
    ...
    report.info(
        "get_patient('INVALID_ID') raised exception — unknown patient IDs are rejected", "DAT-004"
    )
